...
-- [ MUSIC PLAYER BUTTON ARES ] --
local musicBtn = Instance.new("TextButton", main)
musicBtn.Size = UDim2.new(0, 340, 0, 40)
musicBtn.Position = UDim2.new(0, 20, 0, 200)
musicBtn.BackgroundColor3 = Color3.fromRGB(0, 80, 200)
...
local mStroke = Instance.new("UIStroke", musicBtn)
...
-- Music Panel Ares
local musicPanel = Instance.new("Frame", main)
musicPanel.Size = UDim2.new(0, 300, 0, 140)
musicPanel.Position = UDim2.new(1, 10, 0, 0)
musicPanel.BackgroundColor3 = Color3.fromRGB(15, 5, 30)
musicPanel.Visible = false
Instance.new("UICorner", musicPanel)
local mpStroke = Instance.new("UIStroke", musicPanel)
mpStroke.Color = Color3.fromRGB(0, 200, 255)

local idBox = Instance.new("TextBox", musicPanel)
idBox.Size = UDim2.new(0, 260, 0, 35); idBox.Position = UDim2.new(0, 20, 0, 20)
idBox.PlaceholderText = "ID Music"; idBox.BackgroundColor3 = Color3.fromRGB(25, 10, 50)
idBox.TextColor3 = Color3.new(1,1,1); Instance.new("UICorner", idBox)

local playAres = Instance.new("TextButton", musicPanel)
playAres.Size = UDim2.new(0, 260, 0, 45); playAres.Position = UDim2.new(0, 20, 0, 70)
playAres.Text = "▶️ PLAY (BLUE)"; playAres.BackgroundColor3 = Color3.fromRGB(0, 120, 255)
playAres.TextColor3 = Color3.new(1,1,1); playAres.Font = Enum.Font.GothamBold; Instance.new("UICorner", playAres)

-- Animation & Toggle Settings
local elements = {title, closeBtn, targetBox, boxStroke, listBtn, listBtnStroke, flingBtn, fStroke, killBtn, kStroke, viewBtn, vStroke, musicBtn, mStroke}

local function showElements()
    for _, el in ipairs(elements) do
        if el:IsA("GuiObject") then
            el.Visible = true; el.BackgroundTransparency = 1; el.TextTransparency = 1
        end
        if el:IsA("UIStroke") then el.Transparency = 1 end
    end
    task.delay(0.1, function()
...
local function hideElements()
    for _, el in ipairs(elements) do
        TweenService:Create(el, TweenInfo.new(0.26, Enum.EasingStyle.Back, Enum.EasingDirection.In), {BackgroundTransparency = 1, TextTransparency = 1}):Play()
        if el:IsA("UIStroke") then
            TweenService:Create(el, TweenInfo.new(0.26, Enum.EasingStyle.Back, Enum.EasingDirection.In), {Transparency = 1}):Play()
        end
    end
...
toggleBtn.MouseButton1Click:Connect(toggleGUI)
closeBtn.MouseButton1Click:Connect(toggleGUI)
musicBtn.MouseButton1Click:Connect(function() musicPanel.Visible = not musicPanel.Visible; clickSound:Play() end)

playAres.MouseButton1Click:Connect(function()
    local id = idBox.Text:gsub("%D", "")
    pcall(function()
        musicRemote:FireServer("SkateBoard")
        task.wait(0.5)
        musicRemote:FireServer("PickingScooterMusicText", id)
    end)
    clickSound:Play()
    notify("Ares Music Playing: " .. id)
end)
